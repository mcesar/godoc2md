...
{{range .Funcs}}{{$name_html := html .Name}}## <a id="{{$name_html}}">func</a> [{{$name_html}}]({{$.PDoc.ImportPath|srcLink|html}}{{posLink_url $ .Decl}})
{{node $ .Decl | pre}}
{{comment_md .Doc}}
{{example_md $ .Name}}
{{callgraph_html $ "" .Name}}{{end}}
{{range .Types}}{{$tname := .Name}}{{$tname_html := html .Name}}## <a id="{{$tname_html}}">type</a> [{{$tname_html}}]({{$.PDoc.ImportPath|srcLink|html}}{{posLink_url $ .Decl}})
{{node $ .Decl | pre}}
...
{{range .Methods}}{{$name_html := html .Name}}### <a id="{{$tname_html}}.{{$name_html}}">func</a> ({{md .Recv}}) [{{$name_html}}]({{$.PDoc.ImportPath|srcLink|html}}{{posLink_url $ .Decl}})
{{node $ .Decl | pre}}
{{comment_md .Doc}}
...
{{end}}
- - -
Generated by [godoc2md](http://godoc.org/github.com/mcesar/godoc2md)
`
